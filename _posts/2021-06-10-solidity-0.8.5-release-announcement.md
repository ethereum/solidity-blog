...
```
Calling f("12345678") will return "12345678". Calling f("1234567890") will return "12345678".
...
```
Before, it wan't possible to do `bytes4 sig = bytes4(_payload[:4]);`, the way to do the same thing was:
```
bytes4 sig =
	_payload[0] |
	(bytes4(_payload[1]) >> 8) |
...
Details in the [documentation](https://docs.soliditylang.org/en/v0.8.5/yul.html#verbatim).
