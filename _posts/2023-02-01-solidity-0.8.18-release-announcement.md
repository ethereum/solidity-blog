...
The Linux binary for the 0.8.18 release was built against a too recent version of glibc and does thereby not run on older Linux systems. 
...
Both versions of the compiler will behave identically other than the fact that the replaced one only runs on systems with glibc 2.32 or newer.
